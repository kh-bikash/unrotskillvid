...
"""Download a Google font's latin-subset woff2 files and print the @font-face block.
    ...
    python fetch-font.py "Geist" 400 500 600 700 --dir myproject/assets/fonts

A family that is not in the renderer's pre-bundled set makes lint report
`font_family_without_font_face` as an ERROR, and the render would fall back to a system font.
Embedding it explicitly is the fix, and it also keeps distributed/cloud renders from failing
closed on an unreachable Google Fonts.

Paste the printed block into each composition's <style>, before it is used.
"""
...
def main():
    ...
    ap.add_argument("family", help='e.g. "Geist" or "IBM Plex Sans"')
    ap.add_argument("weights", nargs="+", help="e.g. 400 700")
    ap.add_argument("--dir", required=True, help="output directory, e.g. <project>/assets/fonts")
    ...
    print("Re-check vertical metrics after a swap: a face carrying non-Latin metrics has much")
    print("taller line boxes, and display leading under ~1.2 makes word boxes overlap.\n")
